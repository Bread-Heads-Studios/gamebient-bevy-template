...
- Determinism rules for game code are the seven in `docs/replay-verification.md` §"Determinism rules"; the forbidden-names test in `sim.rs` is the mechanical check.
...
`sim::checksum_tick` folds `data.leaderboard_score()` (not `data.score`); `recorder::seal_run` seals `u64::from(data.leaderboard_score())`; `host::report_score`/`report_game_over` report `leaderboard_score()`. `GameData.lives` folding stays (games without `lives` fold what they have — the script's HAND EDIT covers it).
...
- [ ] **Step 2:** `bash ../../libs/gamebient-bevy-template/tools/rollout-replay.sh .` — expect HAND EDITs for `LeaderboardScore` and the `GamePlugin::build` port only. Confirm `Cargo.toml`, `src/lib.rs`, `src/main.rs`, `src/game/mod.rs` (`GamePlugin { headless }`), `build_web.sh`, workflows, `.gitignore`, `assets/info.json` (`verify_url: https://cannonball-putt.vercel.app/verify.zip`).
